...
\\
\\
\textbf{Part 1. Find the Eigenvalues}
\\
\\
The characteristic equation is $\det(A - \lambda I) = 0$.
\begin{align*}
	\begin{vmatrix} -2-\lambda & 2 & -1 \\ 2 & 1-\lambda & -2 \\ -3 & -6 & -\lambda \end{vmatrix} & = 0                                                                                                                 \\
        & = (-2 -\lambda)[(1-\lambda)(-\lambda) - (-2)(-6)] \\
        & \ \ \ - (2)[(2)(-\lambda) - (-2)(-3)] \\
        & \ \ \ + (-1)[(2)(-6) - (1 - \lambda)(-3)] \\
        & = -(\lambda - 5)(\lambda + 3)^2 = 0
\end{align*}
The eigenvalues are $\lambda_{1} = 5$ and $\lambda_{2, 3} = -3_{(2)}$ (with algebraic
multiplicity 2).
\\
\\
\textbf{Step 2. Find Eigenvectors}
...
\[
    \begin{bmatrix} -7 & 2 & -1 \\ 2 & -4 & -2 \\ -3 & -6 & -5 \end{bmatrix} \begin{bmatrix} x \\ y \\ z \end{bmatrix} = \vec{0}
\]
\end{document}
